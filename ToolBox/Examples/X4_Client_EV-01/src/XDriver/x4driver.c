...
#include <sys/time.h>
#include <time.h>
static long last_tv_sec = 0;
static long last_tv_usec = 0;
void x4driver_tic()
{
    //struct timeval tv;
    //struct timezone tz;
    ...
    //last_tv_usec = tv.tv_usec;
    struct timespec tp;
    clock_gettime(CLOCK_MONOTONIC, &tp);
    last_tv_sec = tp.tv_sec;
    last_tv_usec = tp.tv_nsec/1000;
}

static double x4tick_maxtimes = 0;
static double x4tick_sumtimes = 0;
static long x4tick_printtick = 0;
void x4driver_toc()
{
    struct timeval tv;
    //struct timezone tz;
    ...
    long usetimes = 0;
    long usesec = tv.tv_sec - last_tv_sec;
    if(usesec==0)
    {
        usetimes = tv.tv_usec - last_tv_usec;
    }
    else
    {
        usetimes = tv.tv_usec+usesec*1000*1000 - last_tv_usec;
    }

    x4tick_sumtimes = x4tick_sumtimes + usetimes;
    x4tick_maxtimes = usetimes>x4tick_maxtimes?usetimes:x4tick_maxtimes;
    x4tick_printtick ++;
    if(x4tick_printtick>=100)
    {
    //    printf("cur : %d, avg: %.0f, max: %d usecs\n", usetimes, sumtimes/100, maxtimes);
    //    printtick = 0;
    //    sumtimes = 0;
    //    maxtimes = 0;
    }
...
void x4driver_tickstat()
{
    if(!x4tick_printtick)
        x4tick_printtick = 1;
    printf("sum : %.0f, avg: %.0f, max: %.0f usecs, times=%ld\n", x4tick_sumtimes, x4tick_sumtimes/x4tick_printtick, x4tick_maxtimes, x4tick_printtick);
    x4tick_printtick = 0;
    x4tick_sumtimes = 0;
    x4tick_maxtimes = 0;
}
...
int _x4driver_unpack_and_normalize_downconverted_frame(X4Driver_t* x4driver,float* bins_data, uint32_t bins_data_size ,uint8_t * raw_data, uint32_t raw_data_length)
{
    ...
    for (uint32_t i = 0; i<bins_data_size;i++)
    {
        ...
        if(fbin>1)
        {
            int ibindata = (int)bin_data;
            printf("fbin err: %f, bindata=%d(%d,%d), factor=%f, bpc=%d\n", fbin, ibindata,bin_data1,bin_data2, nfactor, x4driver->bytes_per_counter);
            //printf("[%d][%d][%d][%d][%d][%d]\n",
            //    raw_data[raw_data_index],raw_data[raw_data_index+1],raw_data[raw_data_index+2],
            //    raw_data[raw_data_index+3],raw_data[raw_data_index+4],raw_data[raw_data_index+5]);
            docompare = 5;
        }
        /*else if(docompare && fbin<1)
        {
            int ibindata = (int)bin_data;
            printf("fbin correct: %f, bindata=%d(%d,%d), factor=%f, bpc=%d\n", fbin, ibindata,bin_data1,bin_data2, nfactor, x4driver->bytes_per_counter);
            printf("[%d][%d][%d][%d][%d][%d]\n",
                raw_data[raw_data_index],raw_data[raw_data_index+1],raw_data[raw_data_index+2],
                raw_data[raw_data_index+3],raw_data[raw_data_index+4],raw_data[raw_data_index+5]);
            docompare = 0;	
        ...
        if(docompare>0)
        {
            int usebytes = x4driver->bytes_per_counter;
            for(int x=0;x<usebytes;x++)
                printf("[%d]", raw_data[raw_data_index+x]);
            printf("-> %f\n", fbin);
            docompare --;
        }
...
int x4driver_read_frame_bytes(X4Driver_t* x4driver, uint32_t* frame_counter, uint8_t* data, uint32_t length)
{
    ...
    *frame_counter = x4driver->frame_counter;
    ...
    //x4driver->callbacks.spi_write(x4driver->user_reference, &radar_data_addr, 1);
    //x4driver->callbacks.spi_read(x4driver->user_reference, data, x4driver->frame_read_size);
    //x4driver_toc();
